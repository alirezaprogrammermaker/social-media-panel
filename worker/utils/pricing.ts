...
export function usdToToman(
    ...
    markupPercent: string | number | null | undefined = 0
): number {
    ...
    const markup = parseFloat(String(markupPercent ?? '0'));
    const multiplier = Number.isFinite(markup) && markup > -100 ? 1 + markup / 100 : 1;
    return Math.ceil(usd * rate * multiplier);
}
